...
def get_html(fund,type_url):
    ...
    url = "https://www.cmfchile.cl/institucional/mercados/entidad.php?mercado=V&rut={fund}&grupo=&tipoentidad=RGFMU&row=AAAw%20cAAhAABQKHAAN&vig=VI&control=svs&pestania={type_url}"
...
def transform_df_detalle_fondo(df, fund):
    ...
    nuevos_nombres = {
        'R.U.N. Fondo Mutuo': 'run_fm_largo',
        'Nombre Fondo Mutuo': 'nombre_fm',
        'Nombre Corto': 'nombre_corto',
        'Vigencia': 'vigencia',
        'Estado (indica si fondo está liquidado)': 'estado',
        'Tipo de Fondo Mutuo': 'tipo_fm',
        'R.U.T. Administradora': 'run_adm',
        'Razón Social Administradora': 'razon_social_adm',
        'Fecha Depósito Fondo Mutuo': 'fecha_dep_fm',
        'Fecha Ultima Modificación': 'fecha_ult_mod',
        'Fecha Inicio Operaciones': 'fecha_inic_op',
        'Nro. y Fecha de Resolución Aprobatoria': 'nro_resolucion',
    ...
    date_columns = ['fecha_dep_fm', 'fecha_ult_mod', 'fecha_inic_op',
...
def insert_tb_detalle_fondo(df,connection):
    ...
    for index, row in df.iterrows():
        # Aquí debe modificar, asegúrate de que los nombres de las columnas coincidan con la tabla
        sql_query = INSERT INTO detalle_fondo (run_fm,run_fm_largo,nombre_fm,nombre_corto,vigencia,estado,tipo_fm,run_adm,razon_social_adm,fecha_dep_fm,fecha_ult_mod,fecha_inic_op,nro_resolucion,fecha_cumplimiento,fecha_termino,num_registro) VALUES (%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s)  # Reemplaza con los nombres de tus columnas
        # Ejecutar la inserción con los valores de la fila actual
        connection.cursor().execute(sql_query, (row['run_fm'],row['run_fm_largo'],row['nombre_fm'],row['nombre_corto'],row['vigencia'],row['estado'],row['tipo_fm'],row['run_adm'],row['razon_social_adm'],row['fecha_dep_fm'],row['fecha_ult_mod'],row['fecha_inic_op'],row['nro_resolucion'],row['fecha_cumplimiento'],row['fecha_termino'],row['num_registro']))
        # Confirmar la transacción
